...
public interface IMetaBlock {
     ...
     * if(subBlocksArray[meta] != null) { throw new
     * IllegalArgumentException("Meta "+meta+" is already in use in "+blockNameFU); }
     * if(nameMetaMap.get(sb.getUnlocalizedName()) != null) { throw new
     * IllegalArgumentException("Name "+sb.getUnlocalizedName()+" is already in use in "+blockNameFU); }
     * sb.setParent(this); nameMetaMap.put(sb.getUnlocalizedName(), meta); subBlocksArray[meta] = sb; return new
    ...
    /*
     * @Override public void onNeighborBlockChange(World w, int x, int y, int z, Block nb) { int meta =
     * w.getBlockMetadata(x, y, z); this.getSubBlock(meta).onNeighborBlockChange(w, x, y, z, nb);
     * super.onNeighborBlockChange(w, x, y, z, nb); }
     * @Override public int onBlockPlaced(World w, int x, int y, int z, int side, float hitX, float hitY, float hitZ,
     * int meta) { return this.getSubBlock(meta).onBlockPlaced(w, x, y, z, side, hitX, hitY, hitZ, meta); }
     * @Override public int getExpDrop(IBlockAccess world, int metadata, int fortune) { return
     * this.getSubBlock(metadata).getExpDrop(world, 0, fortune); }
     * @Override public int quantityDropped(int meta, int fortune, Random random) { SubBlock sb = getSubBlock(meta);
     * if(sb.dropsSelf()) { return 1; } return sb.quantityDropped(meta, fortune, random); }
     ...
     * @Override public int damageDropped(int meta) { SubBlock sb = getSubBlock(meta); if(sb.dropsSelf()) { return
     * getDistinctionMeta(meta); } return sb.damageDropped(0); }
     * @Override public int getDamageValue(World world, int x, int y, int z) { return getDistinctionMeta(
